...
class SaveSystem {
    ...
    async populateSaveSlots() {
        ...
        if (saves.length === 0) {
            ...
            noSaves.textContent = 'Nenhum save encontrado';
        ...
        saves.forEach((save, index) => {
            ...
            saveItem.innerHTML = `
                <div class="save-header">
                    <h3>Capítulo ${save.chapter} - Cena ${save.scene}</h3>
                    <span class="save-date">${save.dateString}</span>
                </div>
                <div class="save-details">
                    <span class="save-location">${save.location}</span>
                    <span class="save-karma">Karma: ${save.karma}</span>
                    <span class="save-route">${save.currentRoute || 'Neutro'}</span>
                </div>
                <div class="save-progress">
                    <span>Progresso: ${save.completionPercentage}%</span>
                    <span>Tempo: ${save.playTime}</span>
                </div>
            `;
            
            saveItem.addEventListener('click', () => {
                this.loadGame(save.fileName);
                this.hideSaveMenu();
            });
